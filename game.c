...
static void hit_projectile(uint8_t x, uint8_t y);
...
int8_t move_base(int8_t direction) {	
	...
	else if (direction == MOVE_RIGHT && basePosition<FIELD_WIDTH-1){
		basePosition++;
	}
	//Redraw the base
	...
	if(numProjectiles < MAX_PROJECTILES && 
		...
		redraw_projectile(newProjectileNumber, COLOUR_PROJECTILE);
			if (asteroid_at(basePosition,2) != -1)
			{
				...
				remove_projectile(newProjectileNumber);
				add_to_score(1);
			}
		return 1;
	} else {
...
void falling_asteroids(){
	uint8_t i;
	for(i=0; i < numAsteroids; i++) {
		//Retrieve the asteroid position,
		//then move it down by 1
		uint8_t asteroid_x = GET_X_POSITION(asteroids[i]);
		uint8_t asteroid_y = GET_Y_POSITION(asteroids[i])-1;
	
		if (asteroid_y<0)
		{
			//Asteroid reach end of screen
			remove_asteroid(i);
		} 
		else{
			asteroids[i] = GAME_POSITION(asteroid_x, asteroid_y);
			hit_projectile(asteroid_x, asteroid_y);
		}
	}
	hit_base();
	redraw_whole_display();
...
// Check whether the base is hit
// Returns 0 if there is nothing, otherwise return 1
void hit_base() {
	//					1.(basePosition,1)
	// 2.(basePosition-1,0)	(basePosition,0)	3.(basePosition+1,0)
	for (uint8_t i=0; i<numAsteroids;i++)
	{
		uint8_t asteroid_x = GET_X_POSITION(asteroids[i]);
		uint8_t asteroid_y = GET_Y_POSITION(asteroids[i]);
		if ((asteroid_x==basePosition && asteroid_y == 1)
		|| (asteroid_x==basePosition+1 && asteroid_y == 0)
		|| (asteroid_x==basePosition-1 && asteroid_y == 0)
		|| (asteroid_x==basePosition && asteroid_y == 0))
		{
			remove_asteroid(i);
			redraw_base(COLOUR_BASE);
			lose_life();
...
//As the asteroid moves down,
//check if it hits the projectile
static void hit_projectile(uint8_t x, uint8_t y){
...
//Generate new asteroids if required
void add_asteroid(){
	//Check if there's extra space to add asteroids
	uint8_t possible_positions = 0;
	uint8_t replace_y=FIELD_HEIGHT-1;

	for (uint8_t i=0; i<FIELD_WIDTH; i++)
	{
		if (asteroid_at(i,replace_y) == -1)
		{
			possible_positions++;
		}
	}
	
	//If possible, place the new asteroid
	while (possible_positions!=0 && (MAX_ASTEROIDS - numAsteroids)!=0)
	{
		//Randomly generate a new asteroid
		uint8_t replace_x = (uint8_t)(random() % FIELD_WIDTH);
		if (asteroid_at(replace_x,replace_y) == -1)
		{
			//Record the new asteroid to the list, then draw it
			numAsteroids++;
			possible_positions--;
			asteroids[numAsteroids-1] = GAME_POSITION(replace_x,replace_y);
...
static void remove_asteroid(int8_t asteroidNumber) {
	...
	//As an asteroid disappear, append new one
	add_asteroid();
}
